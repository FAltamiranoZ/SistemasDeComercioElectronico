...
@NamedQueries({
    ...
    , @NamedQuery(name = "OrderedProduct.encabezado", query = "SELECT co.id, co.amount, co.dateCreated, co.confirmationNumber, p.id, p.name, op.quantity FROM Product p, OrderedProduct op, CustomerOrder co WHERE op.product.id = p.id AND op.customerOrder.id = co.id")
})
